...
- TRANSLATE runs of English. If two or more English words appear in a row expressing something, translate them into ${tgt.name} (${tgt.script}) — even if they are only part of a sentence, even if surrounded by ${src.name}, and even if introduced by a phrase like "what I'm saying is" or "to tell the truth". Never leave a run of English untranslated, and never write it in Latin script. This rule does NOT apply to a single English word standing alone — those are loanwords, governed by the rule above, and must be kept.
